...
import { SecuritySuggestion, Vulnerability, FileAnalysisResult } from '@/lib/types';

export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { files } = body as { files: FileAnalysisResult[] };
    ...
    const updatedFiles: FileAnalysisResult[] = [];

    for (const file of files) {
      const suggestions: SecuritySuggestion[] = [];

      // Skip files with no vulnerabilities
      if (file.report.vulnerabilities.length === 0) {
        updatedFiles.push({
          ...file,
          suggestions: []
        });
      ...
      const codeFile = (body.originalFiles as any[])?.find((f: any) => f.filename === file.filename);
      const code = codeFile?.content || '';

      // Limit to top 5 vulnerabilities per file to avoid excessive API calls
      const topVulnerabilities = file.report.vulnerabilities.slice(0, 5);

      for (const vulnerability of topVulnerabilities) {
        try {
...
Format your response as JSON with these fields:
{
  ...
  "preventionTips": ["tip1", "tip2", "tip3"]
          ...
          console.error(`OpenAI error for ${file.filename}:`, vulnerability.type, aiError);
    ...
    console.error('Suggest fixes multi error:', error);
    return NextResponse.json(
      { 
